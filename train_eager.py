  ...
  while True:
    ...
    with tf.GradientTape(persistent = True) as tape:
      ...
      d0_loss = 0.5 * (tf.keras.losses.BinaryCrossentropy(from_logits = False)(tf.zeros_like(fake_disc0), fake_disc0) + \
                       tf.keras.losses.BinaryCrossentropy(from_logits = False)(tf.ones_like(real_disc0), real_disc0));
    ...
    if optimizer.iterations % FLAGS.disc_train_steps == 0:
      ...
      optimizer.apply_gradients(zip(g2_grads + g1_grads + g0_grads, gen2.trainable_variables + gen1.trainable_variables + gen0.trainable_variables));
